        ...
        <div class="text-dark container mx-auto rounded-xl bg-white p-8 shadow-lg">
            ...
            <form action="{{ route('events.update', $event) }}" method="POST" enctype="multipart/form-data"
                ...
                <div>
                    ...
                    <input type="text"
                        class="w-full rounded-lg border border-gray-300 px-4 py-2 transition focus:ring-2 focus:ring-indigo-400 focus:outline-none"
                        id="short_description" name="short_description" required placeholder="Enter short description"
                ...
                <div>
                    ...
                    <textarea
                        class="w-full rounded-lg border border-gray-300 px-4 py-2 transition focus:ring-2 focus:ring-indigo-400 focus:outline-none"
                        id="description" name="description" required placeholder="Enter event description">
    {{ old('description', $event->description) }}</textarea>
                ...
                <div>
                    ...
                    <input type="date"
                        ...
                        value="{{ old('event_date', $event->event_date ? \Illuminate\Support\Carbon::parse($event->event_date)->format('Y-m-d') : '') }}" />
                </div>
                <div>
                ...
                <div>
                    ...
                    <select id="status" name="status"
                        class="w-full rounded-lg border border-gray-300 px-4 py-2 transition focus:ring-2 focus:ring-indigo-400 focus:outline-none"
                        required>
                        <option value="" disabled {{ old('status', $event->status) ? '' : 'selected' }}>
                            Select event status
                        </option>
                        <option value="berlangsung" {{ old('status', $event->status) == 'berlangsung' ? 'selected' : '' }}>
                            Berlangsung
                        </option>
                        <option value="rutin" {{ old('status', $event->status) == 'rutin' ? 'selected' : '' }}>
                            Rutin
                        </option>
                ...
                <div>
                    ...
                    <select id="event_category_id" name="event_category_id"
                        ...
                        <option value="" disabled
                            {{ old('event_category_id', $event->event_category_id) ? '' : 'selected' }}>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        @if ($event->thumbnail_path)
                            <div class="flex items-center">
                                <img src="{{ asset('storage/' . $event->thumbnail_path) }}" alt="Current Image"
                                    class="h-20 rounded" />
                            </div>
                        @endif
                    </div>
                    @if ($event->thumbnail_path)
                        <input type="hidden" name="old_thumbnail_path" value="{{ $event->thumbnail_path }}" />
                    @endif
                </div>
                <button type="submit"
